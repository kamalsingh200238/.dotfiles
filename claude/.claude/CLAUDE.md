...
- **VCS Detection:** Before any version control action, detect whether the repo uses `jj` or `git`. Do not assume both.
  Use the repo's real workflow.
- **Prefer jj:** When both `jj` and `git` are present, prefer `jj`.
- **Commit Leaves a Fresh Commit:** When committing, record the current work and ensure a new empty working commit is
  present on top for the next slice (e.g. `jj commit` / `jj new`).
- **Git Policy:**
...
- **Comments:** Add brief comments only for non-obvious logic, edge cases, invariants, or tradeoffs. Explain why the
