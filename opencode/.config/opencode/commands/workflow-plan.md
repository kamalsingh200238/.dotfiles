...
After producing the plan, save it to `.opencode/plans/<slug>.md` in the project root, where `<slug>` is a short kebab-case name you derive from the work request. Create the `.opencode/plans/` directory if it does not exist. If a plan with the same slug already exists, ask the user whether to overwrite, pick a new slug, or abort. Print the saved path on the last line.
