...
- Use `/workflow-plan` before starting a new task.
- Use `/workflow-work` to implement one approved sub-phase.
- Use `/workflow-review` before any commit.
- Use `/workflow-commit` only after review.
...
- Look for bugs, missed requirements, dead code, weak cleanup, regressions, missing tests, missed edge cases,
  and simpler or better approaches.
