...
lsp.on_attach(function(client, bufnr)
	...
	end, { desc = "goto definition" }, bufopts)
	vim.keymap.set("n", "gD", function()
		vim.lsp.buf.declaration()
	end, { desc = "goto declaration" }, bufopts)
	vim.keymap.set("n", "gT", function()
		vim.lsp.buf.type_definition()
	end, { desc = "goto type_definition" }, bufopts)
	vim.keymap.set("n", "gi", function()
		vim.lsp.buf.implementation()
	end, { desc = "goto implementation" }, bufopts)
	vim.keymap.set("n", "gp", "<Cmd>Lspsaga peek_definition<CR>", { desc = "goto peek_definition" }, bufopts)
	vim.keymap.set("n", "gr", function()
		vim.lsp.buf.references()
	end, { desc = "goto references" }, bufopts)
	vim.keymap.set("n", "gl", "<Cmd>Lspsaga lsp_finder<CR>", { desc = "lspsaga lsp finder" }, bufopts)

	vim.keymap.set("n", "<leader>lr", function()
		vim.lsp.buf.rename()
	end, { desc = "traditional rename by lsp" }, bufopts)
	vim.keymap.set("n", "<leader>lR", "<Cmd>Lspsaga rename<CR>", { desc = "rename by lspsaga" }, bufopts)
	vim.keymap.set("n", "<leader>ls", function()
		vim.lsp.buf.signature_help()
	end, { desc = "lsp signature" }, bufopts)
	vim.keymap.set("n", "<space>lc", function()
		vim.lsp.buf.code_action()
	end, { desc = "code actions by lsp" }, bufopts)
	vim.keymap.set("n", "<space>lC", "<cmd>Lspsaga code_action<cr>", { desc = "code actions by lsp" }, bufopts)
	vim.keymap.set("n", "<space>lf", function()
		vim.lsp.buf.format({ async = true })
	end, { desc = "format code" }, bufopts)

	-- workspace
	vim.keymap.set("n", "<space>lwa", function()
		vim.lsp.buf.add_workspace_folder()
	end, { desc = "add workspace" }, bufopts)
	vim.keymap.set("n", "<space>lwr", function()
		vim.lsp.buf.remove_workspace_folder()
	end, { desc = "remove workspace" }, bufopts)
